...
import { View, Text, StatusBar, TouchableOpacity, BackHandler, ScrollView, ImageBackground, ActivityIndicator, TextInput } from 'react-native';
...
const AllTickets = (props) => {
    ...
    let Search;
    Search = item => item.details.toLowerCase().includes(search.toLowerCase());
    ...
    return(
        <View style={{flex:1}}>
            ...
            <View style={{
                ...
                <TextInput value={search} onChangeText={(search) => setSearch(search)} style={{color:Black,paddingLeft:H(1.5)}} placeholder="Search" placeholderTextColor={Grey}/>
                <Ionicons name={'search-outline'} size={22} color={ButtonClr} style={{marginRight:H(1.5)}} />
            </View>
            ...
            {alltickets?.filter(Search).map((item,i) => {
                return(
                    <TouchableOpacity
                    key={i}
